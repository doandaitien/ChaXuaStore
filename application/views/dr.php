...
//  ----------------------------------------------------------------------------
$OID=substr($orderInfo,3,strlen($orderInfo)-3);
$transStatus = "";
if($hashValidated=="CORRECT" && $txnResponseCode=="0"){
	$transStatus = "Giao dịch thành công";
	$sql="update orders set statement='thành công, chưa giao hàng' where OID=".$OID;
	$this->db->query($sql);
}elseif ($hashValidated=="INVALID HASH" && $txnResponseCode=="0"){
	$transStatus = "Giao dịch Pendding";
	$sql='update orders set statement="chờ xử lý của ngân hàng,mã giao dịch".$transactionNo."where OID='.$OID;
}else {
	$transStatus = "Giao dịch thất bại";
	$sql="update orders set statement='thất bại' where OID=".$OID;
	$this->db->query($sql);
}
...
if ($txnResponseCode != "7" && $txnResponseCode != "No Value Returned") {
	...
	?></td>
		...
		<tr>
			<td><a href="<?php  echo base_url();?>index.php/Home_C"> Về cửa hàng</a></td>
		</tr>
